...
MODEL_PATH = "/home/faezeh/Documents/Projects/VirtualMirror/data/shape_predictor_68_face_landmarks.dat"
# Load model
...
# image = args["image"]
# image = cv2.imread("/home/faezeh/Documents/Projects/VirtualMirror/data/input/input.jpg")
window_name = "image"
...
class lenses(object):
    ...
    def getFaceLandmarks(self,image):
    ...
    def drawEyeLandmarks(
        self, im, landmarks, landmarksToDraw, color=(0, 255, 0), radius=1, test=False
    ):
    ...
    # the eye region
    def houghTransformCircleDetector(self, image, eye_radius, eye_distance):
        # parameters
    ...
    def filterOutCircles(self, circles, landmarks):
        circles = np.uint16(np.around(circles))
    ...
    def createHistogramOfColorsInEyeRegion(self, img, circles, eye_radius, test=False):
    ...
    def changeHueOfHistogram(
        self, histogram, eye_pixels, eye_pixel_hue, h, s, v, color, test=False
    ):
        largestBins = self.findLargestNConsecutiveBins(histogram, 30)
        if test:
            print(histogram[0])
            print(largestBins)
        if color == "brown":
    ...
    def findLargestNConsecutiveBins(self, histogram, n):
        bins = histogram[0]
        maxSum = 0
        maxBins = []
    ...
    def changeEyeColor(self, img, eye_color, test=False):
        # grab the landmark points
        landmarks = self.getFaceLandmarks(img)
        points = landmarks.parts()
        ...
        if test:
            self.drawEyeLandmarks(img.copy(), landmarks, leftEyeLandmarks + rightEyeLandmarks)
            print("eye_radius", eye_radius, "eye_distance", eye_distance)

        circles = self.houghTransformCircleDetector(img, eye_radius, eye_distance)
        # drawCircles(img.copy(), circles, eye_radius)
        circles = self.filterOutCircles(circles, points)
        histogram, eye_pixels, eye_pixel_hue, h, s, v = self.createHistogramOfColorsInEyeRegion(
            img, circles, eye_radius, test
        )
        return self.changeHueOfHistogram(
